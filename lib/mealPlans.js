...
export async function getMealPlansByDateRange(startDate, endDate, userId) {
  try {
    ...
    .populate('meals.recipe', 'title category prepTime recipeServings macros ingredients') // Get recipe details
    .populate('cookingSessions.recipe', 'title category prepTime recipeServings ingredients')
    ...
    return mealPlans.map(plan => ({
      ...
      meals: plan.meals.map(meal => ({
        ...
        recipe: {
          ...
          recipeServings: meal.recipe.recipeServings || 1,
          macros: meal.recipe.macros,
          ingredients: meal.recipe.ingredients || []
        },
        notes: meal.notes,
        source: meal.source,
        plannedServings: meal.plannedServings || meal.recipe.recipeServings || 1,
      ...
      cookingSessions: plan.cookingSessions.map(session => ({
        recipe: {
          ...
          recipeServings: session.recipe.recipeServings || 1,
          ingredients: session.recipe.ingredients || []
        },
        notes: session.notes,
        timeSlot: session.timeSlot,
        servings: session.servings,
        plannedServings: session.plannedServings || session.servings || session.recipe.recipeServings || 1,
...
export async function getMealPlanByDate(date, userId) {
  try {
    ...
    const mealPlan = await MealPlan.findOne({ userId, date })
      .populate('meals.recipe', 'title category prepTime recipeServings macros ingredients')
      .populate('cookingSessions.recipe', 'title category prepTime recipeServings ingredients');
...
// Update planned servings for one meal or cooking session entry
export async function updatePlannedServings(date, userId, entryType, index, plannedServings) {
  try {
    await dbConnect();
    ...
    const safePlannedServings = Math.max(1, Number(plannedServings) || 1);

    if (entryType === 'meal') {
      if (!mealPlan.meals[index]) {
        throw new Error('Meal entry not found');
      }
      mealPlan.meals[index].plannedServings = safePlannedServings;
    } else if (entryType === 'cooking_session') {
      if (!mealPlan.cookingSessions[index]) {
        throw new Error('Cooking session entry not found');
      }
      mealPlan.cookingSessions[index].plannedServings = safePlannedServings;
      mealPlan.cookingSessions[index].servings = safePlannedServings;
    } else {
      throw new Error('Invalid entry type');
    }

    await mealPlan.save();
    return await getMealPlanByDate(date, userId);
  } catch (error) {
    console.error('Error updating planned servings:', error);
    throw error;
  }
}

// Toggle exclude/include from shopping list for one entry
export async function updateExcludeFromShopping(date, userId, entryType, index, excludeFromShopping) {
  try {
    await dbConnect();
    ...
    const nextExclude = excludeFromShopping === true;

    if (entryType === 'meal') {
      if (!mealPlan.meals[index]) {
        throw new Error('Meal entry not found');
      }
      mealPlan.meals[index].excludeFromShopping = nextExclude;
    } else if (entryType === 'cooking_session') {
      if (!mealPlan.cookingSessions[index]) {
        throw new Error('Cooking session entry not found');
      }
      mealPlan.cookingSessions[index].excludeFromShopping = nextExclude;
    } else {
      throw new Error('Invalid entry type');
    }
    ...
    console.error('Error updating excludeFromShopping:', error);
    throw error;
  }
}
