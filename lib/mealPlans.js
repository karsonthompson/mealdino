...
// Get meal plans for a date range (e.g., a week)
export async function getMealPlansByDateRange(startDate, endDate) {
  try {
    await dbConnect();

    // Find all meal plans within the date range
...
export async function saveMealPlan(date, meals = [], cookingSessions = []) {
  try {
    await dbConnect();

    // Find existing meal plan or create new one
    let mealPlan = await MealPlan.findOne({ date });
...
export async function addMealToPlan(date, mealData) {
  try {
    await dbConnect();
    ...
    await mealPlan.save();
    return await getMealPlanByDate(date); // Return updated plan
  } catch (error) {
...
// Add a cooking session to a specific date
export async function addCookingSessionToPlan(date, sessionData) {
  try {
    await dbConnect();
...
export async function deleteMealPlan(date) {
  try {
    await dbConnect();
    await MealPlan.findOneAndDelete({ date });
    return true;
  } catch (error) {
