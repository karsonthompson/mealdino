...
const LLM_CHUNK_CHARS = 24000;
const LLM_MAX_CHUNKS = 6;
...
function isPlaceholderLine(line) {
  ...
  return value.includes('needs review') || value.includes('review this imported recipe');
}
...
function scoreDraftQuality(draft) {
  ...
  const realIngredients = ingredients.filter((line) => !isPlaceholderLine(line)).length;
  const realInstructions = instructions.filter((line) => !isPlaceholderLine(line)).length;
  const hasMeaningfulDescription = String(draft?.description || '').trim().length >= 20 ? 1 : 0;
  const hasPrepTime = Number(draft?.prepTime) > 0 ? 1 : 0;
  return (realIngredients * 3) + (realInstructions * 2) + hasMeaningfulDescription + hasPrepTime;
}
...
function dedupeDraftsByTitle(drafts) {
  ...
  for (let index = 0; index < drafts.length; index += 1) {
    ...
    if (!byTitle.has(key)) {
      byTitle.set(key, sanitized);
      continue;
    }

    const existing = byTitle.get(key);
    if (scoreDraftQuality(sanitized) > scoreDraftQuality(existing)) {
      byTitle.set(key, sanitized);
    }
  }
...
async function parseWithLLM({ text, fileName, mimeType }) {
  ...
  for (const block of blocks) {
    ...
    if ((current.length + 2 + block.length) <= LLM_CHUNK_CHARS) {
      current = `${current}\n\n${block}`;
    } else {
      chunks.push(current);
      current = block;
      if (chunks.length >= LLM_MAX_CHUNKS) break;
    }
  }
  if (chunks.length < LLM_MAX_CHUNKS && current.trim()) chunks.push(current);
  if (chunks.length === 0) chunks.push(sourceText.slice(0, LLM_CHUNK_CHARS));
  ...
  for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex += 1) {
    ...
    try {
      ...
      if (!response.ok) {
        allWarnings.push(`LLM chunk ${chunkIndex + 1} failed (${response.status}); using deterministic fallback for that section.`);
        continue;
      }
      ...
      allDrafts.push(...recipes.map((recipe, index) => sanitizeDraftShape(recipe, index)).filter(Boolean));
      allWarnings.push(...warnings);
    } catch (error) {
      console.error('LLM recipe import chunk failed:', { chunkIndex: chunkIndex + 1, error });
      allWarnings.push(`LLM chunk ${chunkIndex + 1} timed out or failed; using deterministic fallback for that section.`);
    } finally {
  ...
  if (allDrafts.length === 0) {
    return { drafts: [], warnings: ['LLM enhancement unavailable for this upload; using deterministic parse only.', ...allWarnings] };
  }

  return {
    drafts: dedupeDraftsByTitle(allDrafts),
